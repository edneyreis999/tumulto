...
export function decideBot(view, playerId, memory, profile) {
  if (view.tick < memory.nextDecisionTick)
    return {
      input: { direction: memory.direction, firePressed: false },
      memory,
    };
  ...
  for (const item of view.items) {
    ...
    if (benefit > 0)
      choices.push({
        cell: item.cell,
        value: benefit / Math.max(1, distance(origin, item.cell)),
      });
  }
  for (let index = 0; index < 64; index++) {
    ...
    if (owner !== playerId && distance(origin, cell))
      choices.push({
        cell,
        value:
          (owner === null ? profile.weights.neutral : profile.weights.rival) /
  ...
  choices.sort(
    (a, b) =>
      b.value - a.value ||
      ((cellIndex(a.cell) + playerId * 17) % 64) -
        ((cellIndex(b.cell) + playerId * 17) % 64),
  );
  ...
  const direction =
    legal.find(
      (d) =>
        target &&
        distance(
          { row: origin.row + DELTAS[d][0], col: origin.col + DELTAS[d][1] },
          target,
  ...
  memory = {
    ...memory,
    nextDecisionTick: view.tick + ticks(view, profile.decisionMs),
    direction,
    target,
  };
  ...
  const firePressed = Boolean(
    ...
    view.players.some((other) => {
      if (other.id === playerId || active(view, other.protectedUntilTick))
        return false;
