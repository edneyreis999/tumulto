...
export function createApp(
  ...
  <section id="fatal" class="screen" hidden><h1>A partida foi interrompida.</h1><p id="fatal-message"></p><p id="fatal-code"></p><p id="fatal-copy-status" role="status"></p><button id="fatal-reload" class="primary">Recarregar</button><button id="fatal-copy">Copiar diagnóstico</button><button id="fatal-menu">Voltar ao menu</button></section>
  ...
  function beginMatch() {
    ...
    memories = match.players.map((p) =>
      p.id ? createBotMemory(match.seed, p.id) : null,
    );
    phase = "playing";
  ...
  async function copy() {
    ...
    try {
      ...
      $(phase === "error" ? "fatal-copy-status" : "pause-reason").textContent =
        "Diagnóstico copiado.";
    } catch {
  ...
  configLoader()
    ...
    .catch(() => {});
  if (initialState) {
    memories = initialState.players.map((p) =>
      p.id ? createBotMemory(initialState.seed, p.id) : null,
    );
